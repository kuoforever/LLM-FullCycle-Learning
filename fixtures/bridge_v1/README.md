...
`valid/runtime-manifest.json` is generated by the offline producer command at
the pinned Runtime commit
`8ace897f746a4aa3dd3f8b10af392ea9ba81941d`.
`valid/minimal-run-export.json` is the smallest complete consumer example.

`fixture-metadata.json` pins the producer commit, pull request, every
schema/contract version, the consumer schema version, and the canonical
manifest digest.

`invalid/` contains fail-closed examples:
...
Tests construct oversized input, checkpoint, and event data from the real
boundary constants so the repository does not need to store a 24 MiB junk
fixture.
